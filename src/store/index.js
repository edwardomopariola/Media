...
export const store = configureStore({
    reducer: {
        ...
        [photosApi.reducerPath]: photosApi.reducer,  //add the photosApi reducer to the store using the reducerPath key
    },

    middleware: (getDefaultMiddleware) => { //add a middleware option that takes a getDefaultMiddleware function as an argument
        return getDefaultMiddleware()
            .concat(albumsApi.middleware)  //add the albumsApi.middleware to the default middleware
            .concat(photosApi.middleware);  //add the photosApi.middleware to the default middleware
    }
});
...
export { useFetchPhotosQuery, useAddPhotoMutation, useRemovePhotoMutation } from './apis/photosApi';  //export the useFetchPhotosQuery hook from the photosApi, the useAddPhotoMutation hook from the photosApi, and the useRemovePhotoMutation hook from the photosApi